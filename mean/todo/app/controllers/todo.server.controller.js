...
var Todo = require('../models/todo.server.model');

exports.list = function(req, res){
    //use mongoose to get all todos in the db
    Todo.find({createdBy: req.session.user._id}).exec( function(err, todos) {
...
exports.create = function(req, res){
	...
	todo.save(function(err){
		if(err)
			...
			res.json({mess: 'todo created !', success: true, todo: todo});
				...
				todo.save(function(err){
					if(err)
					   res.send(err);
						...
						res.json({mess: 'todo updated !', success: true, todo: todo});			
				});
			}
		}			
    });
